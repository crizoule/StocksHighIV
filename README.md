...
**NAAIM Exposure Index.** A sixth card shows the National Association of Active Investment Managers' weekly survey: members' average US equity exposure, from −200% (leveraged short) through 0 (cash) and 100% (fully invested) to +200% (leveraged long). Since August 1, 2026, NAAIM sells current readings by subscription and shows the public a table three months late. The app bundles NAAIM's last free full-history file, 1,047 weeks from July 2006 to July 29, 2026 (`highiv/naaim_history.csv`, personal use), and on each refresh adds any newer week from NAAIM's public table, so the card and its chart series stay about three months behind. The card says so, gives the date the next week becomes public, and the reading is never counted in the Sentiment label. A report saved before this version shows the card and chart straight away, from the bundled history.
...
Each card shows the latest price with its 1-month and 1-year change, the leading group's net position and its three-year COT index (0 = least long, 100 = most long), the second group's net position, and a small chart of price above both. At 80 or above or 20 or below the card reads "Speculators crowded long/short", or "Asset managers unusually long/light" where asset managers lead ("... volatility" for VIX). Beside the full chart at the top, a navigator lists both sections by category; each category opens to show its markets with their COT index, and a label on any market at an extreme (index 95 or above, 5 or below) or near one (80 or above, 20 or below): extreme long, or extreme short, or "light" where asset managers lead. A collapsed category still counts its flagged markets. Choosing a market there, or selecting a card, puts it on the full chart, with the same ranges, log scale and correlation table as the other tabs; the navigator remembers which categories are open. Positions are as of Tuesday and published Friday at 3:30 PM ET; the tab gives the report date, its release and the next one. These shares use all open interest, so the S&P 500 reading differs slightly from the Sentiment tab's COT card, which leaves out spread positions.
