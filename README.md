...
**S&P 500 and sentiment chart.** Below the cards, one chart shows the S&P 500 on top and one sentiment series underneath, on the same dates: the AAII bull–bear spread, VIX, Cboe's equity put/call ratio (5-day average) or CNN Fear & Greed. The replica's history is used when CNN's is unavailable. Buttons switch between 1 month, 3, 6, 1 year, 5 and 10 years. Hovering shows both values for a date. The two lines have separate scales in separate panels, because two y-axes on one plot can make any two lines look related. A table below gives, for every range, the S&P 500's change, the series' average and low–high, and the correlation of its changes with S&P 500 returns between the same dates (weekly for AAII, daily for the others; at least 8 changes). Each series covers what its source provides: S&P 500 and VIX 10 years, AAII since 1987, CNN about 5 years, and put/call the stored Cboe history.
